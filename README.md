...
E2E тесты реализованы в виде отдельной апп-ки `apps/e2e` что бы не тащить тяжелые тестовые зависимости (`testcontainers`
и тому подобное) и не замусоривать основные приложения.

Запуск e2e-тестов:

```
pnpm test:e2e # в корне монорепы
```
